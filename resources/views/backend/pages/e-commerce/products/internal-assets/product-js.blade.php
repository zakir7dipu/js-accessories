    ...
    (function ($) {
        ...
        if ($('#subCategory').val()){
            $.ajax({
                ...
                success:function (data) {
                    $('#proCategory').empty().append(data);
                    let subcategoryId = {{ $selectedItem?
                                                    ($selectedItem->pro_category
                                                    ?$selectedItem->pro_category->id
                                                    :0)
                                                    :0 }};
        ...
        function deleteASingleRow(){
            swal({
                ...
                text: "{{__('Once you delete, You can not recover this Product and related files.')}}",
                icon: "warning",
                dangerMode: true,
                buttons: {
                    cancel: true,
